<?php 
    ...
    if($_SESSION['user']){ //checks if user is logged in
        ...
        header("location:../home/index.php"); // redirects if user is not logged in
    }
    $user = $_SESSION['user']; //assigns user value

	$query2 = mysqli_query($con,"SELECT * from users WHERE username='$user'"); //Query the users table if there are matching rows equal to $username
	...
	$str=fread($myfile,filesize("../data.txt"));
		...
		fclose($file); 
		  ...
		  $myfile = fopen("../data.txt","w");  // to empty that file
			...
			window.location.assign("../home/profile.php");</script>'; // redirects to register.php
			$done_update = mysqli_query($con,"UPDATE users set done=1 WHERE username='$user'"); //Query the users table if there are matching rows equal to $username
		}
		else{
